...
Current Laravel state: `POST /sales` creates open, unpaid sales, snapshots server catalog prices, reserves eligible stock by FEFO, and persists the sale, sale lines, and inventory movements atomically. `POST /sales/{saleId}/items` adds a separate price-snapshot line to an open, unpaid sale and atomically updates its total and stock movements. `DELETE /sales/{saleId}/items/{saleItemId}` marks an unpaid sale line removed, subtracts its saved line total, and restores stock to the exact lots reserved for that line using compensating movements. `POST /sales/{saleId}/mark-paid` changes an open, unpaid sale to completed and paid without changing inventory or recording payment details; retrying the transition returns the current sale. Repeating the same add or remove request does not duplicate stock changes. Create and add operations replay their original responses for repeated idempotency keys. `GET /sales` supports the documented filters and pagination, returning totals, creator IDs, and active line counts. `GET /sales/{saleId}` returns the saved creator ID, state, payment status, total, and line price snapshots. The cancellation path remains a registered placeholder until its implementation slice lands.

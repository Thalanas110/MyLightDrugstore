...
Current Laravel state: `POST /sales` creates open, unpaid sales, snapshots server catalog prices, reserves eligible stock by FEFO, and persists the sale, sale lines, and inventory movements atomically. `POST /sales/{saleId}/items` adds a separate price-snapshot line to an open, unpaid sale and atomically updates its total and stock movements. Both write operations replay the original response for repeated idempotency keys. `GET /sales` supports the documented filters and pagination, returning totals, creator IDs, and active line counts. `GET /sales/{saleId}` returns the saved creator ID, state, payment status, total, and line price snapshots. The other Sales paths remain registered placeholders until their implementation slices land.

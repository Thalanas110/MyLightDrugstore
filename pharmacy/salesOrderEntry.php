...
<div id="content">
	...
	<tr>
		...
		<td>
			<?php
				include('conf.php');
				$result = mysql_query("SELECT * FROM tbldruginfo ORDER BY fldDrugInfoID")or die(mysql_error());				
			?>
				<select name = "itemCode" style = "border-color:red; height:25px; width:185px; text-transform: uppercase;">
					<option selected></option>
					<?php
						while($row = mysql_fetch_object($result)){
						
					?>
						<option style = "font-family:verdana;" value = "<?php echo $row->fldDrugInfoID; ?>"><?php echo $row->fldDrugInfoID; ?> - <?php echo $row->fldGenericName; ?> (<?php echo $row->fldBrandName; ?>)</option>
			<?php
						}
				?>
						...
						else 
							{
								if($yourQuantityVar <= 0)
								...
								else if($realQuantity >= $yourQuantityVar)
									{
										...
										echo "<meta http-equiv='refresh' content='3;url=salesOrderEntry.php'><h4 style = 'text-align:center; color:#000000; font-weight:normal; font-family:verdana; margin-left:0px;'>Customer Order Added Sucessfull<br /><a href = 'salesOrderEntry.php' style = 'margin-left:0px; color:#FF0000'>Click here if it does not redirect in 5 second</a></h4>";
										session_destroy();
								}
